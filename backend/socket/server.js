...
// AI proxy endpoint for Groq API
app.post('/api/ai/chat', async (req, res) => {
    try {
        ...
        console.error('Error proxying AI request:', err);
...
io.on('connection', (socket) => {
    ...
    socket.on('create_voice_room', ({ topic }) => {
        ...
        const roomId = `voice_${Date.now()}_${userId}`;
        const newRoom = {
            id: roomId,
            topic: topic || 'Untitled Room',
            hostId: userId,
        ...
        // Fetch host profile
        // In a real app, you'd fetch this from DB. checking if I have profile in waitingUsers or need to fetch
        // For now, minimal info

        console.log(`[VoiceRoom] Created ${roomId} by ${userId}`);

        socket.emit('voice_room_created', {
            roomId,
            topic: newRoom.topic
        });

        // Broadcast to all to update lobby
    ...
    socket.on('get_voice_rooms', () => {
        const roomsList = Array.from(voiceRooms.values()).map(r => ({
            id: r.id,
            topic: r.topic,
            hostId: r.hostId,
            participantsCount: r.participants.size,
            speakersCount: r.speakers.size,
            // We could add avatar previews here later
        }));
    ...
    socket.on('leave_voice_room', ({ roomId }) => {
        ...
        if (room && userId) {
            ...
            if (room.participants.size === 0) {
                ...
                console.log(`[VoiceRoom] Room ${roomId} deleted (empty)`);
            } else {
